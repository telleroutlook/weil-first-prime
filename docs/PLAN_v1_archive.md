...
## 状态更新（2026-08-06 深夜）——同构映射战役 + L=0.42 判决

### 战略探索：7 大同构映射方向全部关闭
详见 `docs/EXTENDED_GOALS.md`。为寻找把 Weil 判据 $(\star)$ 映射到"已证领域"的
同构（唯一逻辑封闭的通往 RH 之路），系统性排查了 7 个方向，全部关闭：
性质T、Bakry-Émery、Lee-Yang、费米子/LSM、Ruelle双曲、自旋玻璃/Parisi、准周期/KAM。
死因分别是结构前提崩塌 / 非对称标度膨胀 / 退随机化障碍 / 实空间局域化——
**没有一个死于循环或换记号**。结论：$\lambda(L)$ 符号是全谱不可约量，无结构捷径；
$(\star)$ 这个"无限公式"早已存在，稀缺的是它的证明（= RH）。

**两颗无条件、无 RH 的副产品种子**（均为猜想/待证，非成果）：
1. 基态节点相变作为 λ 符号翻转的"前兆指标"（需第二窗口数据标定）；
2. frustration 渐近不可消解（用无条件 log-Chowla，独立负结果候选，E1 同气质）。

### ⚠️ 精度可靠性教训（必须记住）
上述战役中的多数"数值处决"用了**探路精度** `integrate_M_K` depth=2（秒级）。
后续 L=0.42 判决扫描暴露：depth=2 在高 N **非单调、不稳定**
（N=16 得 −0.0007，N=20 得 −0.022，自相矛盾）。因此：
- 纯逻辑判决（Parisi 系综短路）不受影响；
- 符号类判决（Lee-Yang frustration）稳健但欠 certify 级符号复核；
- **一切定量数字（+84%/+16%、节点位置、能隙、IPR）已降级为"定性倾向，欠 certify 级复核"**。
探路工具只配探路，不配下判决——这是本仓库 fail-closed 精神对自身的一次纠正。

### L=0.42 判决：certify 级已确认判负（2026-08-07 凌晨）
**结论：L=0.42 用 Legendre 基，certify 级 min_eig ≈ −0.022，确定过不了。**

| 精度 | N | min_eig | 说明 |
|---|---|---|---|
| 快扫 depth=2 | 16 | −0.000686 | **假象**（乐观 30 倍）|
| 快扫 depth=2 | 20 | −0.021959 | — |
| **certify depth4/3+Arb** | **16** | **−0.021806** | **真值** |
| certify depth4/3+Arb | 20 | ⏳ 运行中 | 二次确认 |

关键发现：certify N=16(−0.0218)与快扫 N=20(−0.0220)吻合，暗示真实 min_eig
在 N=16~20 已**趋稳于 −0.022**，非趋 0。缺口 0.022 远超每步改善量(+0.002~0.003)，
**加大 N 到 32/40 也补不回**。

**精度教训坐实**：depth=2 把真值 −0.022 算成 −0.0007（误差 30 倍）。这确认了
EXTENDED_GOALS 里对所有 depth=2 定量判决的降级是必要的。之前"倾向判负"结论
碰巧正确，但当时的理由（基于 −0.0007）是错的——现在有了 certify 级的正确理由。

**触发分支**：Legendre 在第一窗口深水区结构性不足 → 启动 Prolate P3（下节）。

### ⚠️ 关键澄清（2026-08-07 凌晨，自主修正）：充分非必要

Schur 分裂残差判据是正定性的充分条件，不是必要条件。min_eig = −0.022
只证明"此证书构造在 N=16 未能认证 λ(0.42)>0"，绝不证明 λ(0.42) ≤ 0。
由于 RH 已在极大高度数值验证为真，λ(0.42) 几乎必然真的为正；失败的是
我们的证书，不是真相。

正确的下一步框架：
- L=0.42 不是真实障碍，而是证书强度不足。
- Prolate P3 的真实目标不是"把负变正"（换基不可能做到），而是
  构造更紧的证书，捕捉保守的分裂残差界所遗漏的那部分正性。
- 具体候选：(a) 优化 η（当前固定 1/2，未扫描）；(b) 更紧的 R_η 分裂；
  (c) prolate 基让同精度用更少维数 → 可负担更大 N → 证书收紧。

这是本夜最诚实、也最有价值的一个认知修正：几乎把"证书失败"误报成了
"结构死刑"。RH 为真时 λ(0.42)>0，我们只是还没造出够强的证书去认证它。

### 下一步真实计划（按确定性排序，非兴奋度）
1. **[论文·第一优先]** FP-0.35 是唯一确定成果 → 发 arXiv endorsement 请求
   （Suzuki/Groskin），进入实质投稿。
2. **[代码·依赖判决]** 待 L=0.42 certify 判决：
   - 若确认过不了 → Legendre 在第一窗口深水区不足 → 启动 Prolate P3
     （带限求积换 S_KK 内核，零范式代价，目标降到 ~12 维；$2c/\pi\sim12$ 已估）。
   - 若翻盘可过 → 现有框架收掉，得"整第一窗口"更强定理。
3. **[理论·锁抽屉]** log-Chowla frustration 种子：仅在头脑冷静时做可行性闸门
   （查清需要 Chowla 的哪个**已证**形式、误差率是否够）；不满足即弃，不倒贴。
...
**当前决策等待点（2026-08-06 18:45）**：

N=32 收敛诊断正在运行（PID=29058，约 2 小时）。

收敛诊断数据（L=0.42，even 扇区）：
...
4 点拟合：L_inf ≈ −0.012，r ≈ 0.84（< 0.917 → 倾向负渐近线，但 4 点无法定论）

**N=32 结果决定下一步**：
- 若 min_eig(N=32) ≈ −0.005 ～ 0（温和减速）→ 精确算 3-5 个坏 entry + rank-2 其余，N≈38 可认证
- 若 min_eig(N=32) ≈ −0.015（强减速，L_inf < −0.005）→ Legendre 基在此 L 结构性泄漏，停止算力投入，转 Route 3

**不做的事**（κ_L² 全矩阵余项）：纸面预判 min_eig − κ_L² ≈ −0.024 − 1.58 ≈ −1.6，必败，不跑。
| 路线 | 状态 | 说明 |
|---|---|---|
