...
### L=0.42 判决：待 certify 级确认（进行中）
**快扫"倾向判负"结论已撤回**（建立在自相矛盾的 depth=2 数据上）。
正用 `scan_lambda_profile` 的 certify 精度（depth=4/3 + Arb）重跑 L=0.42 的
N=16 / N=20，结果出来前**不下 L=0.42 生死结论**。
