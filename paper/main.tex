...
(from Suzuki~\cite{Suzuki2026} equation~(4.5)).
...
\begin{remark}[Dependence on Suzuki's framework and explicit conventions]
\label{rem:conventions}
The following three facts from Suzuki~\cite{Suzuki2026}
(equations (1.1), (2.5), (2.11) and Theorem~1.3) are used throughout:
...
\item \textbf{Kinetic operator.}
  ...
  is specifically $H_d G_{dd}^{-1} \cdot G_{dd} = H_d$ where $d$ is the
...
\item \textbf{Weil constant.} $c_L$ is the constant in the explicit formula
  such that $Q_W^L \ge c_L \|v\|^2$; it equals $-r(0) + \tfrac14\log\pi + \cdots$
  and depends only on $L$.
  The exact formula is $c_L = \log(2\pi L) + \gamma_E$,
  where $\gamma_E = 0.5772\ldots$ is the Euler--Mascheroni constant;
  this follows from Suzuki~\cite{Suzuki2026} equation~(4.5) after
  scaling.  At $L = 7/20$:
  \[
...
\end{enumerate}
These three facts are localised here so that any revision of Suzuki's
conventions affects only this remark and the numerical parameters in §\ref{sec:conjecture}.
\end{remark}
...
(rational upper bound $1355726/993009$, certified to 256-bit Arb precision;
see \cref{rem:conventions}(ii)).
