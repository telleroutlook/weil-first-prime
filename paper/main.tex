...
\item \textbf{Weil constant.} $c_L$ is the constant in the explicit formula
  ...
  and depends only on $L$.  In our computations we use $c_L = 0$.
  This is the weakest assumption: the pilot conditions verify
  $T + V + K_L + \mathcal{P}_{2,L} \ge \Lzero I$ (no $c_L$ subtracted),
  which implies $\lambda(L) \ge \Lzero - c_L$.
  For $\lambda(L) > 0$ it suffices that $c_L < \Lzero = 2^{-30}$,
  or equivalently that the matrix conditions are re-run with the true $c_L$.
  The pilots in \S\ref{sec:conjecture} should therefore be understood as
  establishing $T + V + K_L + \mathcal P_{2,L} \ge \Lzero I$, not directly
  $\lambda(7/20) > 0$.
...
Numerically, the relative bound $\sup_{v\ne0}\ip{Vv}{v}/\ip{Tv}{v} \approx 1.54$
(computed over a 60-term Legendre basis), so $V$ is relatively form-bounded with
respect to $T$ with bound $\rho \approx 1.54$; it is \emph{not} infinitesimally
form-bounded.
...
For the Schur criterion the relevant lower bound is
$\qbar_L[v] \ge b_L \|v\|^2$, where
$b_L = H_d - c_L - \Lzero - \kappa_L > 0$ is established computationally.
Semi-boundedness of $\qbar_L$ on $D(\qbar_L)$ follows from $T \ge 0$,
$V \ge 0$, the boundedness of $K_L$ and $\mathcal P_{2,L}$, and the positivity
of $b_L$.
\end{proof}
