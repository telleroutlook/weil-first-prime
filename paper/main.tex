...
\item \textbf{Weil constant.} $c_L$ is the constant in the explicit formula
  ...
  and depends only on $L$.  In our computations we use $c_L = 0$ (conservative).
...
$D(\qbar_L) = \{v \in L^2(I_L) : \ip{Tv}{v} + \ip{Vv}{v} < \infty\}$,
and all matrix operations in Theorem~\ref{thm:schur} take place within this domain.
\end{lemma}
...
In the $N$-dimensional basis $\{P_{n_k}\}_{k=1}^N$:
\[
  \sum_{k=1}^\infty \bigl|\ip{\mathsf{Q}_N(V+K_L)p}{\phi_k}\bigr|^2
  = p^* S^{(0)} p - p^* (M^{(0)})^* G^{-1} M^{(0)} p
  = p^* R_0\, p,
\]
where $\{\phi_k\}$ is an orthonormal basis of $\operatorname{Ran}\mathsf{Q}_N$,
and the $G^{-1}$ factor arises because the Legendre basis is orthogonal
but not orthonormal ($G_{ii} = 2/(2n_i+1)$).
