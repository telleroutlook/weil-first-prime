...
\item \textbf{Weil constant.} $c_L$ is the constant in the explicit formula
  ...
  In our computations we set $c_L = 0$, so the pilot conditions verify
  $T + V + K_L + \mathcal{P}_{2,L} \ge \Lzero I$ without any $c_L$ subtraction.
  This implies $\lambda(L) = \inf Q_W^L/\|v\|^2 \ge \Lzero - c_L$.

  \textbf{Open verification item:} The value of $c_L$ at $L = 7/20$ is not
  computed in this paper.  Establishing $\lambda(7/20) > 0$ from the pilot
  results therefore requires one of:
  \begin{enumerate}[label=(\alph*),leftmargin=2em,topsep=2pt,itemsep=1pt]
    \item[(a)] Proving or computing $c_L(7/20) < \Lzero = 2^{-30}$
              (highly plausible numerically, but not shown here); or
    \item[(b)] Re-running the matrix conditions of Theorem~\ref{thm:schur}
              with the certified value of $c_L(7/20)$ (part of the O2 programme).
  \end{enumerate}
  The pilots in \S\ref{sec:conjecture} are best understood as establishing
  $T + V + K_L + \mathcal P_{2,L} \ge \Lzero I$, a conditional step
  toward $\lambda(7/20) > 0$.
