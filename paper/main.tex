...
\item \emph{Theorem~\ref{thm:rational-cert}} (pure-rational certificate)
  ...
  whose integer-arithmetic steps are machine-verified in Lean~4 using
  \texttt{native\_decide}. The five verified comparisons are the
  computationally non-trivial core of the argument; the algebraic
  reductions leading to them are carried out by hand in the proof.
...
\begin{proof}
The kernel $V(x) = -\tfrac{1}{2}\log(1-x^2)$ satisfies $0 \le V(x) \le -\log|1-|x||$
near $\pm 1$. By the Hardy--Rellich inequality on $(-1,1)$, $V$ is relatively
form-bounded with respect to the one-dimensional Laplacian (which controls $T$
up to a compact perturbation), giving the stated estimate. The constant
$C_\delta$ can be computed explicitly from the Legendre harmonic numbers $H_n$.
The Archimedean kernel $K_L$ is Hilbert--Schmidt, hence relatively compact
and $\delta$-bounded for any $\delta > 0$. The closed form $\qbar_L$ is therefore
defined on $D(T^{1/2})$ by the KLMN theorem.
\end{proof}
