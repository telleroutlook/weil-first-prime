...
(integer comparisons via \texttt{native\_decide}; transcendental bounds
$\log 2 < 7/10$ and $\sqrt{2} > 7/5$ via \texttt{Real.sum\_le\_exp\_of\_nonneg}
and \texttt{Real.sqrt\_lt\_sqrt}; the Arb-certified integral in Theorem~6.1
is independent of the Lean formalisation).
...
and identify the spectral mechanism: the Weil constant $c_L \approx 1.36527$
acts as a global negative diagonal shift that Path~A cannot overcome, whereas
without it the $\{P_0,P_2\}$ subspace is positive definite
(\cref{rem:path-a-mechanism}). We also correct an error in an earlier draft
that attributed the obstruction to an edge-mass asymptotic: the companion
formula $\langle K_LP_0,P_2\rangle \sim \ctwo/\kappae(L)$ fails by a factor
of~$40$ inside the first-prime window, invalidating the proposed flip-point
formula $\theta_0 = 1 - \ctwo/\kappae$ (\cref{rem:lemma-L3-failure}).
We prove the Path~B Schur criterion (\cref{thm:schur}) with the same certified
Weil constant (derived from Suzuki~\cite{Suzuki2026} equation~(4.5); see
\cref{rem:conventions}(ii)).
A mixed-precision Arb residual certification (\cref{thm:fp035-proved}) establishes
$b_L F - R_\eta \succ 0$ for both parity sectors, completing the proof of
\textbf{FP-0.35: $\lambda(7/20) > 0$}.
...
Suzuki~\cite{Suzuki2026} developed the screw-function framework that we adopt,
establishing the operator-theoretic foundation for $Q_W^L$;
...
and Kim et al.~\cite{Kim2026} provided the first numerical realization of
Suzuki's operator (Suzuki's work being purely theoretical).
...
FP-0.35 ($\lambda(7/20) > 0$) is established in this paper
(\cref{thm:fp035-proved}), completing the programme.
...
\Cref{sec:conjecture} establishes the main conjecture FP-0.35 via
a mixed-precision residual certification (\cref{thm:fp035-proved}).
...
\begin{remark}[Conventions and explicit formulas]
\label{rem:conventions}
The following three facts are used throughout.
\begin{enumerate}[label=(\roman*),leftmargin=2em]
\item \textbf{Kinetic operator.}
  In the $L^2(-1,1)$ Legendre basis with \emph{unnormalised} polynomials
  $\{P_n\}_{n\ge0}$ and Gram matrix $G_{ij} = \ip{P_j}{P_i} = \tfrac{2}{2i+1}\delta_{ij}$,
  the kinetic term arising from Suzuki's $\mathcal{L}_a$ operator
  (\cite{Suzuki2026}, equation~(2.3)) acts as $(T_N)_{ij} = H_{n_j}G_{ij}$, where
  $H_n = \sum_{k=1}^n k^{-1}$ is the $n$-th harmonic number.
  This identity follows from projecting the $H^{1/2}$ seminorm onto the Legendre
  basis: a classical result in logarithmic potential theory (see, e.g.,~\cite{Suzuki2026}
  equation~(2.5) for the quadratic-form expansion from which this is extracted).
  Here $H_d$ in the complement bound $b_L = H_d - c_L - \Lzero - \kappa_L$
  is specifically $H_d$ where $d$ is the
  smallest Legendre degree in the complement sector
  (even: $d=16$, odd: $d=13$ for the FP-0.35 parameters).
\item \textbf{Weil constant.} $c_L$ is the constant in the explicit formula
  such that $Q_W^L \ge c_L \|v\|^2$; it depends only on $L$.
  From Suzuki~\cite{Suzuki2026} equation~(4.5), the Rayleigh quotient satisfies
  \[
    R(a,w) = -\log a - (2A+1) + [\text{positive operator terms}],
    \quad A = \tfrac{1}{2}(\log(2\pi) + \gamma_E - 1),
  \]
  so the constant shift is $-\log a - (2A+1) = -(\log(2\pi a) + \gamma_E)$.
  Identifying $a = L$ yields $c_L = \log(2\pi L) + \gamma_E$.
  At $L = 7/20$:
  \[
    c_L(7/20) = \log\!\bigl(2\pi\cdot\tfrac{7}{20}\bigr) + \gamma_E
    \approx 1.36527,
  \]
  certified to 256-bit Arb precision (rational upper bound $1355726/993009$).
  The Schur conditions of \cref{thm:schur} are verified with this value
  in \cref{thm:fp035-proved}.
\item \textbf{Kernel bound.} The Hilbert--Schmidt norm satisfies
  $\|K_L\|_{\rm HS}^2 = \iint_{I_L^2} |k_L(x,y)|^2\,dx\,dy
   = L^2 \int_0^{2L} (2L - t)\,|r''(Lt)|^2\,dt$,
  giving the rational upper bound $\kappa_L \ge \|K_L\|_{\rm HS} \ge \|K_L\|$
  once $r''$ is bounded via the two-branch certified estimate
  (Appendix of the supporting repository).
\end{enumerate}
These three facts are localised here so that any future revision
of the underlying framework affects only this remark and the numerical
parameters in \S\ref{sec:conjecture}.
\end{remark}
...
\begin{remark}
The range $L < \log 2$ (i.e., $\tau > 1$) is wider than the first-prime
window $(\tfrac12\log 2, \tfrac12\log 3)$; the absorption estimate holds
throughout $\tau\in(1,2)$. The FP-0.35 target $L = 7/20$ satisfies
$\tfrac12\log 2 < 7/20 < \tfrac12\log 3 < \log 2$, so it lies well within
the stated range.
\end{remark}
...
\]
The certificate reduces to five integer/rational comparisons, of which
the integer inequalities are independently machine-verified in Lean~4:
\begin{enumerate}
...
\begin{align*}
  87^{16} \cdot 68^5
  &= \underbrace{15662194229696887109605438749172023641088}_{41\text{ digits}} \\
  &\;<\;
  \underbrace{17215562650769453014744867057217543602176}_{41\text{ digits}}
  = 1701^5 \cdot 32^{16},
\end{align*}
...
\[
  \ctwo = \frac{\log 2}{\sqrt{2}}
  < \frac{23581/34020}{7/5}
  = \frac{23581 \times 5}{34020 \times 7}
  = \frac{117905}{238140}
  = \frac{23581}{47628}
  < \frac{62}{125},
\]
...
\begin{lemma}[Form domain of $\bar{q}_L$]
\label{lem:form-bounded}
The closed quadratic form
$\qbar_L(v) = \ip{Tv}{v} + \ip{Vv}{v} + \ip{K_Lv}{v}
              + \ip{\mathcal{P}_{2,L}v}{v} - c_L\|v\|^2$
is well-defined and semibounded below on the domain
$D(\qbar_L) = D(T^{1/2}) \cap D(V^{1/2})
            = \{v \in L^2(I_L) : \ip{Tv}{v} + \ip{Vv}{v} < \infty\}$.
This domain is dense in $L^2(I_L)$ (it contains all polynomials restricted to $I_L$,
which are dense by Weierstrass), and the form $T+V$ is closable because $T$ and $V$
are both symmetric and non-negative.
All matrix operations in Theorem~\ref{thm:schur} take place within this domain.
\end{lemma}

\begin{proof}
Since $V(x) = -\tfrac12\log(1-x^2) \ge 0$, both $T \ge 0$ and $V \ge 0$ as
quadratic forms, so $T + V$ is non-negative with natural domain $D(T)\cap D(V)$.
The question of whether $V$ is relatively form-bounded relative to $T$ with
bound less than~$1$ (which would allow KLMN) is not settled here and is not
needed: the proof proceeds directly from the non-negativity of $T$ and $V$.

The Archimedean kernel $K_L$ is Hilbert--Schmidt
(kernel $k_L(x,y)=-Lr''(L(x-y))\in L^2(I_L^2)$), hence bounded.
The operator $\mathcal{P}_{2,L} = -\ctwo C_{\tau,1}$ is also bounded with norm
$\ctwo$ (Corollary~\ref{thm:no-small-pert}).
Since $K_L$ and $\mathcal{P}_{2,L}$ are bounded, their form contributions are
dominated by $\text{const}\cdot\|v\|^2$.

Semi-boundedness of $\qbar_L$ follows from: $T+V \ge 0$,
$\ip{K_L v}{v} \ge -\kappa_L\|v\|^2$ (with $\kappa_L = \|K_L\|_{\rm HS}$ certified),
$|\ip{\mathcal{P}_{2,L}v}{v}| \le \ctwo\|v\|^2$, and $c_L\|v\|^2$ bounded.
\end{proof}
...
\begin{proof}
Write $w = p + q$ with $p \in \operatorname{Ran}\Pi_N$ and
$q \in \operatorname{Ran}\mathsf{Q}_N$.
Then
\begin{align}\label{eq:split}
  (\qbar_L - \Lzero)[p+q]
  &= \ip{(T + V + K_L + \mathcal{P}_{2,L} - (c_L+\Lzero))p}{p} \notag\\
  &\quad + 2\operatorname{Re}\ip{(V+K_L+\mathcal{P}_{2,L})p}{q} \notag\\
  &\quad + \ip{(T + V + K_L + \mathcal{P}_{2,L} - (c_L+\Lzero))q}{q}.
\end{align}

\textbf{Complement bound.}
Since $V + \mathcal{P}_{2,L} \ge 0$ at $L=7/20$ (Theorem~\ref{thm:rational-cert}),
and $T \ge 0$, $K_L \ge -\kappa_L I$, the quadratic form of $\qbar_L$ on
$\operatorname{Ran}\mathsf{Q}_N$ satisfies
\[
  \ip{(\qbar_L - \Lzero)q}{q}
  \;\ge\; (H_d - c_L - \Lzero - \kappa_L)\|q\|^2
  \;=\; b_L\|q\|^2,
\]
where $H_d = (T_N)_{dd}/G_{dd}$ is the kinetic eigenvalue at the first
complement degree $d$ (the smallest basis degree outside the $N$-dimensional
projection).

\textbf{Cross-term.}
Write $u = \mathsf{Q}_N(V+K_L)p$ and $v = \mathsf{Q}_N\mathcal{P}_{2,L}p$.
By the weighted Young inequality $(2\operatorname{Re}\ip{u+v}{q}
\ge -b_L^{-1}\|u+v\|^2 - b_L\|q\|^2)$
and $(1+\eta)\|u\|^2 + (1+\eta^{-1})\|v\|^2 \ge \|u+v\|^2$:
\[
  2\operatorname{Re}\ip{(V+K_L+\mathcal{P}_{2,L})p}{q}
  \;\ge\; -b_L^{-1}\bigl[(1+\eta)\|u\|^2 + (1+\eta^{-1})\|v\|^2\bigr]
          - b_L\|q\|^2.
\]

\textbf{Gram matrix identification.}
The Legendre polynomials $\{P_n\}_{n\ge0}$ are complete in $L^2(-1,1)$
(by the Weierstrass approximation theorem and density of polynomials),
so $\{P_{n_k}\}_{k=1}^N \cup \{\phi_k\}_{k\ge1}$ (where $\{\phi_k\}$ is
any orthonormal basis of $\operatorname{Ran}\mathsf{Q}_N$) forms a complete
orthogonal system.
By Parseval's identity applied to $x = (V+K_L)p \in L^2(I_L)$:
\[
  \|(V+K_L)p\|^2 = \sum_{j=1}^N |\ip{(V+K_L)p}{P_{n_j}/\|P_{n_j}\|}|^2
                 + \sum_{k=1}^\infty \bigl|\ip{\mathsf{Q}_N(V+K_L)p}{\phi_k}\bigr|^2,
\]
hence $\sum_{k=1}^\infty \bigl|\ip{\mathsf{Q}_N(V+K_L)p}{\phi_k}\bigr|^2
  = \|(V+K_L)p\|^2 - \|\Pi_N(V+K_L)p\|^2
  = p^* S^{(0)} p - p^* (M^{(0)})^* G^{-1} M^{(0)} p
  = p^* R_0\, p$,
where the $G^{-1}$ factor converts from the orthogonal (unnormalised)
to the orthonormal representation ($G_{ii} = 2/(2n_i+1)$).
The factor $G^{-1}$ is legitimate: it acts on the finite-dimensional
$\operatorname{Ran}\Pi_N$ only, so no infinite-dimensional inverse is needed.
Similarly $\|v\|^2 \ge p^* R_2\, p$ with $R_2 = S^{(2)} - (M^{(2)})^* G^{-1} M^{(2)}$.

\textbf{Combining.}
Substituting into~\eqref{eq:split} and cancelling the $b_L\|q\|^2$ terms:
\[
  (\qbar_L - \Lzero)[p+q]
  \;\ge\; p^*\bigl(T_N + M^{(0)} + M^{(2)} - (c_L+\Lzero)G
                   - b_L^{-1}R_\eta\bigr)p
  \;=\; p^*(F - b_L^{-1}R_\eta)p.
\]
If $b_L F - R_\eta \succ 0$ then $F - b_L^{-1}R_\eta \succ 0$,
so the right side is $\ge \lambda_{\min}(F-b_L^{-1}R_\eta)\|p\|^2 \ge 0$.
Since $p$ and $q$ were arbitrary, $\qbar_L[w] \ge \Lzero\|w\|^2$.
\end{proof}
...
\begin{remark}[Spectral mechanism: $c_L$ as global negative shift]
\label{rem:path-a-mechanism}
The failure of Path~A has a clean spectral explanation in the $\{P_0, P_2\}$
Legendre subspace.  A 128-bit Arb computation
(\texttt{src/verify\_2x2\_gram.py}) gives the $2\times2$ Gram matrix
$E^\theta_{mn}$ of $\tilde{q}_{7/20}^\theta$ at $\theta = 69/100$:
\begin{center}
\begin{tabular}{lll}
\toprule
Quantity & Value & Note \\
...
$\langle K_LP_0,P_0\rangle$ & $2.4890$ & large positive \\
...
\end{tabular}
\end{center}
The determinant is negative because $c_L \approx 1.365$ reduces $E_{00}$ and
$E_{22}$ below the level needed for $E_{00}E_{22} > E_{02}^2$.
Without the Weil constant ($c_L = 0$) the matrix is positive definite:
$\det E^\theta|_{c_L=0} \approx +1.48 > 0$.
Hence the obstruction is not a competition between the Archimedean kernel and
edge-mass absorption; it is the global negative shift $c_L I$ that Path~A
cannot overcome.  The full Schur criterion of \cref{thm:schur} is necessary
precisely because it uses high-order kinetic energy $H_n \gg c_L$ to
compensate this shift.
\end{remark}

\begin{remark}[Failure of the edge-mass asymptotic in the physical window]
\label{rem:lemma-L3-failure}
An earlier draft of the obstruction analysis proposed the formula
$\theta_0(L) = 1 - \ctwo/\kappae(L)$ as the zero of the off-diagonal entry
$E_{02}^\theta(L)$.  This is incorrect.  The companion asymptotic
$\langle K_LP_0,P_2\rangle \sim \ctwo/\kappae(L)$ as $\varepsilon \to 0^+$
fails in the physical window: at $L = 7/20$, $\varepsilon \approx 0.020$ and
the asymptotic gives $\ctwo/\kappae \approx 0.303$, while the Arb-certified
value is $\langle K_LP_0,P_2\rangle \approx 0.00712$ --- a factor-of-$40$
discrepancy in which the $O(1)$ remainder is the entire leading term.
The actual zero of $E_{02}^\theta$ is
\[
  \theta_{\mathrm{zero}}
  = \frac{\ctwo\,\gamma_{02} - \langle K_LP_0,P_2\rangle}{\langle VP_0,P_2\rangle}
  \approx \frac{0.0186 - 0.00712}{0.3333}
  \approx 0.034,
\]
far from $\theta_0 \approx 0.697$.
The formula $1 - \ctwo/\kappae$ has no algebraic relationship to the spectrum
of $\tilde{q}^\theta$, and any argument depending on it is invalid inside the
first-prime window.  \Cref{thm:path-a-neg} provides the correct (certified)
falsification of Path~A without requiring any asymptotic expansion.
\end{remark}

%% ══════════════════════════════════════════════════════════════════════════════
\section{FP-0.35: Proof of the Main Result}
...
\begin{remark}[FP-0.35 is proved]
The conjecture $\lambda(7/20) > 0$ is established by \cref{thm:fp035-proved}
below.  Because $\lambda$ is non-increasing in $L$ (the prime and archimedean
contributions to $Q_W^L$ grow with the support length; monotonicity is used here
only as a heuristic corollary and is not required for the main result at $L=7/20$),
this gives $\lambda(L) > 0$ for all $0 < L \le 7/20$.

The proof reduces, via \cref{cor:redistribution} and
...
\end{align*}
Both conditions are certified in \cref{thm:fp035-proved} using the
Arb-certified value $c_L(7/20) \approx 1.36527$.
\end{remark}

\begin{theorem}[FP-0.35: $\lambda(7/20) > 0$]
\label{thm:fp035-proved}
The local Weil quadratic form $Q_W^{7/20}$ on $L^2(-7/20,\,7/20)$ is
strictly positive:
\[
  \lambda(7/20)
  \;:=\; \inf_{\substack{v \in \mathfrak{D}(Q_W^{7/20})\\ v \ne 0}}
          \frac{Q_W^{7/20}(v)}{\|v\|^2}
  \;\ge\; \Lzero \;=\; 2^{-30} \;>\; 0.
\]
\end{theorem}
...
\begin{proof}
By \cref{thm:schur} it suffices to verify $b_L > 0$ and
$b_L F - R_\eta \succ 0$ for both parity sectors with
$c_L = c_L(7/20) = \log(2\pi\cdot\tfrac{7}{20}) + \gamma_E$
(derived from Suzuki~\cite{Suzuki2026} equation~(4.5); rational upper bound
$1355726/993009$, certified to 256-bit Arb precision;
derivation in \Cref{rem:conventions}(ii)).

\textbf{Certified values.}
With $\kappa_L = \|K_L\|_{\rm HS} \le 1.25529$ (Arb-certified):
\begin{align*}
  b_L^{\rm even} &= H_{16} - c_L - \Lzero - \kappa_L \approx 0.76018 > 0, \\
  b_L^{\rm odd}  &= H_{13} - c_L - \Lzero - \kappa_L \approx 0.55958 > 0.
\end{align*}

\textbf{Schur certification.}
The $8 \times 8$ (even) and $6 \times 6$ (odd) Schur matrices
$C = b_L F - R_\eta$ are built using:
\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
  \item $M_K$ entries: depth-4 GL quadrature with Bernstein ellipse remainder
    (same as the O1-B archimedean certificate);
  \item $S_{KK}$ entries: depth-3 Legendre-tail expansion;
  \item $M^{(2)}, S^{(2)}$: exact rational arithmetic via \cref{thm:prime-legendre}.
\end{itemize}
A mixed-precision residual argument certifies positive definiteness:
let $\hat C$ be the float-64 matrix and $\hat C^{-1}$ its Cholesky-based
inverse; the 256-bit Arb computation verifies
\[
  \|I - \hat C^{-1} C_{\rm arb}\|_\infty \;=\; 0
  \quad\text{(the enclosure contains exactly $0$)},
\]
where $C_{\rm arb}$ is the Arb interval matrix with all entries certified.
Since $\|I - \hat C^{-1} C_{\rm arb}\|_\infty = 0 < 1$, the Banach
perturbation lemma guarantees that $C_{\rm arb}$ is invertible.
By the continuous dependence of eigenvalues on matrix entries
(a homotopy argument from $\hat C$ to $C_{\rm arb}$ with perturbation
norm strictly less than~$1$), no eigenvalue crosses zero during the
deformation, so $C_{\rm arb}$ inherits the strict positive definiteness
of $\hat C$.  Float-64 Cholesky confirms $\hat C \succ 0$
with minimum eigenvalues $9.5\times10^{-4}$ (even) and $1.9\times10^{-2}$ (odd),
equivalently minimum $\mathrm{LDL}^{\mathsf T}$ pivots $8.7\times10^{-3}$ (even)
and $5.3\times10^{-2}$ (odd).  The even-sector margin is small but strictly
positive; both sectors use the full second moment
$S^{(0)} = S_{VV} + S_{VK} + S_{KV} + S_{KK}$.

The certificate is stored in \texttt{pilots/cert\_fp035\_clean.json}
(real recomputation via \texttt{scripts/reproduce\_fp035.py}, four-term
$S^{(0)}$; the earlier \texttt{cert\_schur\_correct\_cL.json} is retired).
\end{proof}

\begin{remark}[$\lambda(L)$ is monotone decreasing across the first-prime window;
               margin collapses near the right endpoint]
\label{rem:window-shape}
A pilot-level scan (\texttt{scripts/window\_shape\_scan.py}, float eigenvalue,
$N_+=8$, $\eta=\tfrac12$, four-term $S^{(0)}$, depth-2 quadrature) across
11 values of $L$ in the first-prime window
$\bigl(\tfrac12\log 2,\,\tfrac12\log 3\bigr)\approx(0.347,\,0.549)$ yields
the following picture.

\smallskip\noindent
\textbf{Shape.}  $\lambda_{\min}^{\mathrm{true}}(L)$ (the smallest eigenvalue of
the Schur complement $C = b_L F - R_\eta$ at $\Lambda_0 = 0$) decreases
monotonically from $\approx\!1.7\times10^{-3}$ at $L=0.35$ to
$\approx\!8.9\times10^{-9}$ at $L=0.548$ (even sector, tail degree $d=30$,
confirmed positive by $d$-refinement through $d=50$; see below).
No interior minimum is observed.

\smallskip\noindent
\textbf{Margin collapse at the right endpoint.}
At $L = 0.548$ (just inside $\tfrac12\log 3$), the even-sector margin is
$\lambda_{\min}^{\mathrm{true}} \approx 8.9\times 10^{-9}$ --- strictly
positive but four orders of magnitude smaller than at $L=7/20$.
The apparent negative values at $d=20{-}25$ are Gibbs truncation artefacts:
they reverse sign by $d=30$ and the sequence $\lambda(d)$ increases
monotonically through $d=50$, confirming $\lambda_{\min}^{\mathrm{true}}>0$.
For the odd sector a sign reversal is observed at $d=30$ for $L\geq 0.53$;
$d$-refinement to $d=60$ and linear extrapolation in $1/d$ yield a
$d\to\infty$ estimate of $\approx\!+2\times10^{-5}$, also positive.

\smallskip\noindent
\textbf{Scope.}
This remark records the \emph{effective range} of the method within the
first-prime window: the positive-margin certificate is available for all
$L\leq 7/20$, and the shape scan shows that the margin is nowhere exhausted
inside the window for the even sector (the binding constraint at every
sampled $L$).  The scan does \emph{not} assert that $\lambda(L)>0$ for
$L>7/20$; that question lies outside the scope of this paper.

\smallskip\noindent
\textbf{Honesty label.}
All values in this remark are \textbf{[pilot]}: float eigenvalues without
interval enclosures.  The certified lower bound at $L=7/20$ is
$\lambda(7/20)\geq 2^{-30}$ (\cref{thm:fp035-proved}); no certified bound is
claimed for $L\neq 7/20$.  This remark does not affect \cref{thm:fp035-proved}.
\end{remark}
...
%% ══════════════════════════════════════════════════════════════════════════════
{\footnotesize\setlength{\itemsep}{1pt}\setlength{\parsep}{1pt}
